...
download = BashOperator(
    ...
    bash_command='python3 /opt/airflow/dags/scripts/download.py',
    dag=dag,
)
train = BashOperator(
    task_id='train',
    depends_on_past=False,
    bash_command='python3 /opt/airflow/dags/scripts/train.py',
    retries=3,
    dag=dag,
)
serve_commands = """
    lsof -i tcp:8008 | awk 'NR!=1 {print $2}' | xargs kill;
    python3 /opt/airflow/dags/scripts/serve.py serve
    """
serve = BashOperator(
    task_id='serve',
    depends_on_past=False,
